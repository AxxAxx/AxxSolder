...
{
	UG_FillFrame(0, 0, 239, 29, C_Background);
	LCD_PutStr(5, 5, (char *)title, FONT_arial_20X23,
	           C_Footer, C_Background);
...
{
	uint16_t y = 35 + line * 26;
	UG_DrawFrame(0, y - 3, 239, y + 23, C_Header);
	UG_DrawFrame(1, y - 2, 238, y + 22, C_Header);
}

static void erase_cursor(uint8_t line)
{
	uint16_t y = 35 + line * 26;
	UG_DrawFrame(0, y - 3, 239, y + 23, C_Background);
	UG_DrawFrame(1, y - 2, 238, y + 22, C_Background);
}
...
{
    const uint16_t fg = C_Text;
    const uint16_t bg = C_Background;
		...
		if (editing && edit_val) {
			...
			if (delta != prev_delta) {
				...
				*edit_val = old_value + (float)delta;
				...
				draw_prof_value(vis, val_buf, C_Background, C_Text);
				draw_cursor(vis);
			}
		}
	...
	while (running) {
		...
		if (redraw) {
			...
			LCD_PutStr(239 - (int)strlen(cnt) * 13, 5, cnt,
			           FONT_arial_20X23, C_Footer, C_Background);
			...
			for (uint8_t i = 0; i < menu_lines_on_screen && (page_start + i) < list_len; i++) {
				...
				if (li < count) {
					...
					if (active_for_current == li) {
						uint16_t y = 35 + i * 26;
						LCD_PutStr(200, y, "[A]", FONT_arial_20X23, C_Header, bg);
					}
				} else if (can_add && li == add_idx) {
					draw_line(i, "+ Add New", C_Footer, bg);
				} else {
					draw_line(i, "Back", C_MenuDim, bg);
				}
			}
			draw_cursor(cursor - page_start);
			prev_cursor = cursor;
...
void profiles_popup(enum handles h)
{
	...
	UG_DrawFrame(popup_x, popup_y, popup_x + popup_w, popup_y + popup_h, C_Header);
	UG_DrawFrame(popup_x + 1, popup_y + 1, popup_x + popup_w - 1, popup_y + popup_h - 1, C_Header);
	UG_DrawFrame(popup_x + 2, popup_y + 2, popup_x + popup_w - 2, popup_y + popup_h - 2, C_Header);
	...
	while (1) {
		...
		if (cursor != prev_cursor) {
			...
			UG_DrawFrame(popup_x + 3, cy - 2, popup_x + popup_w - 3, cy + 22, C_Header);
			UG_DrawFrame(popup_x + 4, cy - 1, popup_x + popup_w - 4, cy + 21, C_Header);
			prev_cursor = cursor;
